...
| `/day` | Single focused column, grouped list | The full picture, one tap away: habits due, overdue, due today, anytime. Second pass: **the spine and the hand.** The "start here" pick became the page's hero (`start-here.tsx`) — accent-washed, display serif, and a live time line ("in 28 min · at 15:00", ticking with the calendar's own 30-second now rhythm; static until the first client tick so hydration stays honest). The 12px PlanStrip became the **day rail** (`day-rail.tsx`): hour ticks with mono labels every third hour, kind-coloured spans, the up-next pick pinned to its own minute, and the running-blue now-line wearing a chip that says what time it is — the one number the line could never speak. The block layer draws in once on load, the same single orchestrated moment the calendar's week strips hold. Section counts speak the scheduling panel's lesson: "2 left", a number that only goes down; overdue keeps its clay count. Third pass: **the quest system.** The rail came out and the day's plan became a game's quest list, because the plan had been rendered twice (hero for habits, rail for blocks) while the hero itself stayed blind to the calendar — a block blocked out at 15:00 could never answer "what's active / what's next". `lib/day-quests.ts` merges the day's blocks and the anchored habits not already spoken for into one timeline (a habit scheduled into a block is claimed by the block — the scheduling panel's own rule), reads each quest's state against the current minute, and picks the hero by precedence: active work quest → active recovery → arrived habit → free roam (buffer-only) → next quest within the old 120-minute lookahead → the available backlog → all done. The hero (`quest-hero.tsx`) renders the state it's handed: the **active quest** card carries the block's objectives as tickable lines with the calendar's own block-scoped tick (`toggleBlockTask`), so finishing the last line completes the quest and the hero hands over to whatever comes next — the reward loop the checkbox pop already serves, now with a horizon. A recovery window gets its own card in rest's colour with no draining bar (a countdown would turn rest into work). **Free roam** names the next quest and its countdown, so buffer time has a horizon instead of being a void. The **next quest** card puts the unlock countdown ("unlocks in 1h 20m · at 15:00") on its own mono line. The **quest log** (`quest-log.tsx`) lists every timed thing with game states — ✓ complete, ● in its window, ○ unlocking in N minutes, past-unticked dimmed — and the header keeps the count that only climbs: "2/5". Sections wear the same vocabulary: **Dailies** (habits — the things that reset daily), **Quests** (due today), **Side quests** (anytime), **Completed**, each with a done/total counter; overdue keeps its clay count. The game grammar is deliberately thin — states, unlock times, a counter — no points, no XP, no celebration animation; it feeds the ticking reward loop the app already trusts. No new colour: the running blue stays reserved for actual running timers, and an active quest wears teal because a plan in its window is still a plan. |
...
The calendar's third pass was polish rather than restructure, three small
truth-tellings:

- **The now-line speaks.** A mono chip in the line's own colours rides it
  through today's column (day and week alike), because a line says "you are
  here" but never says "here is 11:52" — and doing that arithmetic was the
  header's job for exactly the wrong audience. Still the only running blue on
  the screen; the reservation holds.
- **The waking window is shaded.** The grid runs all 24 hours but the plan can
  only live in 07–22 — the same window the header strips measure — so the
  hours outside it dim behind a hairline on each edge. The usable day is now
  visible without a single word.
- **The empty day points at the gesture.** In day view with no blocks, a
